...
const useLaboratoryMngmtStore= defineStore('laboratoryMngmt',()=>{
    ...
    function isLabAdmin(labId) {
        ...
        return lab && lab.adminUserId === authStore.currentUser.id;
    }
    ...
    function isLabMember(labId) {
        if (!authStore.currentUser) return false;
        const lab = laboratories.value.find(l => l.id === labId);
        return lab && lab.memberUserIds && lab.memberUserIds.includes(authStore.currentUser.id);
    }
    ...
    function addLaboratory(laboratory) {
        ...
        if (authStore.currentUser) {
            laboratory.adminUserId = authStore.currentUser.id;
            ...
            if (laboratory.labResponsibleId && 
                parseInt(laboratory.labResponsibleId) !== authStore.currentUser.id &&
